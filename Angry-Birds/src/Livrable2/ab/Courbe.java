...
public class Courbe {
	...
	public Point courbeBez(Point p, Point p1, Point p2, double t) {
		...
		rep.x = (int) (((1 - t) * (1 - t)) * p.x + 2 * t * (1 - t) * p1.x + (t * t) * p2.x);
		rep.y = (int) (((1 - t) * (1 - t)) * p.y + 2 * t * (1 - t) * p1.y + (t * t) * p2.y);
		return rep;
	}
	...
	public Point courbePhysique(double t, double angle) {
		...
		if (o.getX() > 130) {
			p.x = (int) ((vitesse * Math.cos(angle) * t) + o.getX() - frottement);
		} else {
			p.x = (int) ((vitesse * Math.cos(angle) * t) + o.getX());

		}
		p.y = (int) ((((90) * Math.pow(t, 2)) + vitesse * Math.sin(angle) * t + o.getY()) - frottement);
		return p;

	}

	public Point courbeObstacle(double vitesse, double t, double angle) {
		Point p = new Point(0, 0);
